  ...
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/generate') {
      try {
        ...
        const zaiRequest = {
          model: body.model || 'cogvideox-3',
          prompt: body.prompt
        };

        // Add model-specific parameters
        if (body.model?.startsWith('viduq1')) {
          zaiRequest.duration = body.duration || 5;
        ...
        // Extract video URL from video_result array if present
        let videoUrl = videoData.video_url;
        if (!videoUrl && videoData.video_result && videoData.video_result[0]) {
...
<body class="bg-background text-foreground min-h-screen">
  <div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <form id="videoForm" class="space-y-6">
      ...
      <div class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-3">
          <button type="button" data-model="cogvideox-3" class="model-btn border-2 border-primary bg-primary/10 rounded-lg p-3 text-left hover:bg-primary/20 transition">
            ...
            <div class="text-xs text-muted-foreground">Up to 4K • 30/60fps • Audio</div>
          </button>
          <button type="button" data-model="viduq1-text" class="model-btn border-2 border-border rounded-lg p-3 text-left hover:bg-accent transition">
            ...
            <div class="text-xs text-muted-foreground">5s • 24fps • General/Anime</div>
          </button>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div class="space-y-2">
          <label class="text-sm font-medium">Start Frame</label>
          <div id="startZone" class="border-2 border-dashed border-border rounded-lg p-8 text-center cursor-pointer hover:border-primary/50 transition min-h-[150px] flex items-center justify-center">
            <input type="file" id="startFile" accept="image/*" class="hidden">
            <div class="upload-content">
              <div class="text-4xl mb-2">📸</div>
              <p class="text-sm text-muted-foreground">Drop or click</p>
            </div>
            <img id="startPreview" class="hidden max-h-[200px] rounded-lg">
          </div>
        </div>
        <div class="space-y-2">
          <label class="text-sm font-medium">End Frame</label>
          <div id="endZone" class="border-2 border-dashed border-border rounded-lg p-8 text-center cursor-pointer hover:border-primary/50 transition min-h-[150px] flex items-center justify-center">
            <input type="file" id="endFile" accept="image/*" class="hidden">
            <div class="upload-content">
              <div class="text-4xl mb-2">📸</div>
              <p class="text-sm text-muted-foreground">Drop or click</p>
            </div>
            <img id="endPreview" class="hidden max-h-[200px] rounded-lg">
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    function setupUpload(zoneId, fileId, previewId, target) {
    ...
    setupUpload('startZone', 'startFile', 'startPreview', 'start');
    setupUpload('endZone', 'endFile', 'endPreview', 'end');
